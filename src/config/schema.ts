...
export const RateLimitConfigSchema = z.object({
  maxRequests: z
    .number()
    .int()
    .positive('maxRequests debe ser un entero positivo'),
  windowSeconds: z
    .number()
    .int()
    .positive('windowSeconds debe ser un entero positivo'),
});

// Esquema para timeouts de las rutas
export const RouteTimeoutConfigSchema = z.object({
  connect: z.number().int().positive('El timeout de conexión debe ser un entero positivo').optional(),
...
export const RouteConfigSchema = z.object({
  prefix: z
    ...
    .refine(
      (val) => val.startsWith('/'),
      'El prefijo de la ruta debe comenzar con "/"'
    )
    .refine(
...
export const OverrideConfigSchema = z.object({
  path: z
    ...
    .refine(
      (val) => val.startsWith('/'),
      'El path del override debe comenzar con "/"'
    )
    .refine(
      (val) => val === '/' || !val.endsWith('/'),
...
export const GatewayConfigSchema = z.object({
  ...
  routes: z
    ...
    .min(1, 'Debe haber al menos una ruta configurada'),
